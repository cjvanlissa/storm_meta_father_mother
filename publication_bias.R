...
dfCI$rhs <- c("Child", "Father", "Child")[dfCI$model]
dat$lhs <- c("Mother", "Mother", "Father")[dat$model]
dat$rhs <- c("Child", "Father", "Child")[dat$model]
...
write.csv(cbind(Parameter = paste0("R(", c("mother, child", "mother, father", "father, child"), ")"),
                t(sapply(models, function(x){ regtest(x)[3:4]})),
                FailsafeN = sapply(models, function(x){ fsn(x$yi, x$vi)$fsnum})), "eggerstest.csv")
